// Vercel Serverless Function — POST /api/update-settings
const CORS_HEADERS = {
    'Access-Control-Allow-Origin': '*',
    'Access-Control-Allow-Methods': 'POST, OPTIONS',
    'Access-Control-Allow-Headers': 'Content-Type, Authorization',
};

export default async function handler(req, res) {
    Object.entries(CORS_HEADERS).forEach(([k, v]) => res.setHeader(k, v));
    if (req.method === 'OPTIONS') return res.status(200).end();
    if (req.method !== 'POST') return res.status(405).json({ success: false, error: 'Method Not Allowed' });

    // Admin Security Check
    const authHeader = req.headers.authorization || '';
    if (!authHeader.startsWith('Bearer ') || authHeader.split(' ')[1] !== process.env.ADMIN_SECRET_TOKEN) {
        return res.status(401).json({ success: false, error: 'Unauthorized' });
    }

    try {
        const payload = req.body;
        if (!payload) throw new Error("No payload provided");

        // We assume settings table has id=1
        const keys = Object.keys(payload).filter(k => k !== 'id');
        if (keys.length === 0) return res.status(200).json({ success: true });

        const setClause = keys.map(k => `${k} = ?`).join(', ');
        const params = keys.map(k => {
            let val = payload[k];
            // Convert objects/arrays to JSON strings
            if (val !== null && typeof val === 'object') {
                return JSON.stringify(val);
            }
            return val;
        });

        // Add ID for WHERE clause
        params.push(1);

        const query = `UPDATE settings SET ${setClause} WHERE id = ?`;

        const d1Res = await fetch(
            `https://api.cloudflare.com/client/v4/accounts/${process.env.CF_ACCOUNT_ID}/d1/database/${process.env.CF_DB_ID}/query`,
            {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${process.env.CF_WRITE_TOKEN}`
                },
                body: JSON.stringify({ sql: query, params })
            }
        );

        const data = await d1Res.json();
        if (!data.success) throw new Error(data.errors?.[0]?.message || 'D1 Error');

        // Check if rows were actually updated (if not, maybe we need to insert it)
        const results = data.result?.[0]?.results || [];
        const changes = data.result?.[0]?.meta?.changes || 0;
        
        if (changes === 0) {
            // Might need to insert if not exists
            const insertCols = keys.join(', ');
            const insertPlaceholders = keys.map(() => '?').join(', ');
            const insertParams = [...params.slice(0, -1)]; // everything except the WHERE id
            
            // Add ID to insert
            const insertQuery = `INSERT INTO settings (id, ${insertCols}) VALUES (1, ${insertPlaceholders})`;
            await fetch(
                `https://api.cloudflare.com/client/v4/accounts/${process.env.CF_ACCOUNT_ID}/d1/database/${process.env.CF_DB_ID}/query`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${process.env.CF_WRITE_TOKEN}`
                    },
                    body: JSON.stringify({ sql: insertQuery, params: insertParams })
                }
            );
        }

        return res.status(200).json({ success: true });
    } catch (err) {
        return res.status(500).json({ success: false, error: err.message });
    }
}
