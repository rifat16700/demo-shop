...
<body class="w-full min-h-screen sm:h-auto sm:p-12 sm:flex sm:items-center sm:justify-center">
    ...
    <div class="up-container max-w-md overflow-hidden mx-auto p-8 relative sm:bg-white sm:rounded-lg sm:shadow-lg sm:shadow-[#0057d0]/10 sm:min-w-[650px] sm:flex sm:flex-wrap">
        <div class="zp-hide-on-mobile-pay w-full h-12 shadow-md shadow-[#0057d0]/5 rounded-lg overflow-hidden flex justify-between items-center p-5 bg-white sm:bg-[#fbfcff]  sm:shadow-none sm:ring-1 sm:ring-[#0057d0]/10">
            <div class>
                <a href="<?= BASE_SITE; ?>">
        ...
        <div class="w-full mb-8">
            <div class="zp-hide-on-mobile-pay flex flex-col sm:flex-row flex-wrap mt-5 sm:justify-between sm:items-center">
            ...
            <!-- Legacy fields needed for verify payload -->
            ...
            <?php if ($is_mobile && $acc_tp != 'merchant') : ?>
                ...
                <!-- Other Mobile wallets Theme Layout -->
                <?php else : ?>
                    <style>
                        .rocket-theme-container {
                            background: <?= $brand_bg ?>;
                    ...
                    <div class="rocket-theme-container">
                        <img src="<?= BASE_SITE . payment_option($gateway_type) ?>" class="rocket-logo-img" alt="<?= $brand_name ?>">
                        <div class="rocket-merchant-card">
                            <div><strong>Merchant:</strong> <?= $all_info['brand_name'] ?></div>
                            <div class="mt-1"><strong>Invoice Number:</strong> <?= $all_info['transaction_id'] ?></div>
                            <div class="mt-1"><strong>Amount:</strong> BDT <?= currency_format($all_info['total_amount']) ?></div>
                        </div>
                        <!-- Stage 1 -->
                        <div id="rocket-stage-1">
                            <label class="rocket-label">Enter Your <?= $brand_name ?> Account Number</label>
                            <input type="tel" class="rocket-input zp-phone-generic" placeholder="e.g. 01XXXXXXXXX" maxlength="11" autocomplete="off" autofocus>
                            <div class="rocket-btn-row">
                                <button type="button" class="rocket-btn rocket-btn-secondary" onclick="window.location.href='<?= base_url("api/execute/" . $all_info['tmp_ids']); ?>'">Cancel</button>
                                <button type="button" class="rocket-btn rocket-btn-primary zp-btn-proceed-generic" disabled>Proceed</button>
                            </div>
                        </div>
                        <!-- Stage 2 -->
                        <div id="rocket-stage-2" style="display:none;">
                            <div class="rocket-steps-card">
                                <p>প্রেরক নম্বর: <strong class="zp-phone-display" style="color:#ffd700;"></strong></p>
                                <hr style="border:0; border-top:1px solid rgba(255,255,255,0.2); margin:8px 0 12px;">
                                <ol class="rocket-ol">
                                    <li>আপনার <strong><?= $brand_name ?></strong> অ্যাপ খুলুন অথবা ডায়াল কোডে যান (<?= $brand_dial ?>)।</li>
                                    <li><strong>"<?= $acc_tp_text ?>"</strong> অপশন সিলেক্ট করুন।</li>
                                    <li>নিচের নাম্বারে টাকা পাঠান:
                                        <div class="rocket-copy-box">
                                            <span><?= $target_number ?></span>
                                    ...
                                    <li>টাকার পরিমাণ লিখুন: <strong>BDT <?= currency_format($all_info['total_amount']) ?></strong></li>
                                    <li>পেমেন্ট শেষ হলে নিচে <strong>Verify</strong> চাপুন।</li>
                                </ol>
                            </div>
                            <div class="rocket-btn-row mt-4">
                                <button type="button" class="rocket-btn rocket-btn-secondary" onclick="showStage1Generic()">Back</button>
                                <button type="button" class="rocket-btn rocket-btn-primary payment_submit_done" id="payment_submit_done" data-tmp_id="<?= session('tmp_ids') ?>" data-href="<?= base_url('api/save_payment/' . $gateway_type . '?acc_tp=' . (service('request')->getGet('acc_tp') ?? '')) ?>">Verify</button>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <script>
                    ...
                    $(document).ready(function() {
                        ...
                        $('.zp-phone-generic').on('input', function() {
                            var val = $(this).val().replace(/[^0-9]/g, '');
                            $(this).val(val);
                            var proceedBtn = $(this).closest('.zp-premium-checkout, .bkash-theme-container, .nagad-theme-container, .upay-theme-container, .rocket-theme-container').find('.zp-btn-proceed-generic');
                        ...
                        $('.zp-btn-proceed-generic').on('click', function() {
                            var container = $(this).closest('.zp-premium-checkout, .bkash-theme-container, .nagad-theme-container, .upay-theme-container, .rocket-theme-container');
                    ...
                    function showStage1Generic() {
                        clearAllIntervals();
                        var container = $('.zp-phone-generic').closest('.zp-premium-checkout, .bkash-theme-container, .nagad-theme-container, .upay-theme-container, .rocket-theme-container');
            ...
            <?php else : ?>
                <!-- Fallback layout for Bank, Binance, etc. -->
                <div class="method">
                    <!-- Transaction Id -->
                    <div class="brand-bg p-5 mt-3 rounded-lg overflow-auto">
                        <div class="text-center transaction mt-3">
                            ...
                            <!-- Since input field has same id, let's keep it here -->
                            <script>$('#transaction_id').attr('type', 'text').addClass('font-bangla appearance-none w-full text-[15px] rounded-[10px] sm:bg-[#fbfcff] shadow shadow-[#0057d0]/5 px-5 py-3.5 placeholder-[#94A9C7] focus:outline-none focus:ring-1 focus:ring-[#0057d0]').attr('placeholder', 'ট্রান্সজেকশন আইডি দিন').attr('maxlength', '14').attr('required', 'true').attr('autocomplete', 'off').attr('autofocus', 'true'); $('.transaction').append($('#transaction_id'));</script>
                        </div>
    ...
    <div id="retryModal" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(15,23,42,0.85); backdrop-filter:blur(6px); z-index:9999; align-items:center; justify-content:center;">
        <div style="background:rgba(30,27,75,0.95); border:1px solid rgba(255,255,255,0.1); border-radius:16px; padding:28px 24px; max-width:340px; width:90%; text-align:center; box-shadow:0 20px 60px rgba(0,0,0,0.5); animation:retryBounce 0.4s ease;">
            ...
            <p style="color:#94a3b8; font-size:13px; line-height:1.6; margin-bottom:16px;">পেমেন্ট এখনো প্রসেস হচ্ছে।<br><span id="retryCountdownText" style="color:#fbbf24; font-weight:600;"><span id="retrySeconds">10</span> সেকেন্ড</span> অপেক্ষা করে আবার চেষ্টা করুন।</p>
            <div style="width:100%; height:4px; background:rgba(255,255,255,0.06); border-radius:10px; overflow:hidden; margin-bottom:16px;"><div id="retryProgress" style="height:100%; width:100%; background:linear-gradient(90deg,#fbbf24,#f59e0b); border-radius:10px; transition:width 1s linear;"></div></div>
            <button id="retryVerifyBtn" disabled style="width:100%; padding:14px; border:none; border-radius:10px; font-size:14px; font-weight:700; cursor:not-allowed; background:rgba(251,191,36,0.2); color:rgba(251,191,36,0.5); transition:all 0.3s ease;">⏳ অপেক্ষা করুন...</button>
        </div>
    </div>
    <style>
        ...
        #retryVerifyBtn:not(:disabled){background:linear-gradient(135deg,#fbbf24,#f59e0b)!important;color:#000!important;cursor:pointer!important;box-shadow:0 4px 15px rgba(251,191,36,0.3);}#retryVerifyBtn:not(:disabled):hover{filter:brightness(1.1);transform:translateY(-1px);}</style>
    ...
    <script>
        ...
        function doVerify(element, isSilent) {
            ...
            var url = element.attr("data-href"),
                ...
                data = $.param({ token:token, tmp_id:tmp_id, transaction_id:transaction_id, t_type:t_type, acc_tp:acc_tp });
            
            if (!isSilent) {
                element.prop("disabled", true);
                element.html('🕥 যাচাই হচ্ছে...');
                $("#loader").css("display", "flex");
            }
            ...
            setTimeout(function() {
                $.post(url, data, function(_result) {
                    if (_result.status === 'success' || _result.status === 'warning') {
                        ...
                        if (_result.redirect) { setTimeout(function(){ window.location.replace(_result.redirect); }, 1000); }
                    } else {
                        if (!isSilent) {
                            failAttempts++;
                            if (failAttempts === 1) {
                                $("#loader").css("display","none"); element.prop("disabled",false); element.html('VERIFY');
                                showRetryModal(element);
                            } else {
                                showToast(_result.message, _result.status);
                                if (_result.redirect) { setTimeout(function(){ window.location.replace(_result.redirect); }, 1000); }
                                else { element.prop("disabled",false); element.html('VERIFY'); $("#loader").css("display","none"); }
                            }
                        }
                    }
                }, 'json').fail(function(){ 
                    if (!isSilent) {
                        showToast('Network error','error'); element.prop("disabled",false); element.html('VERIFY'); $("#loader").css("display","none"); 
                    }
                });
            }, delay);
        }
        function showRetryModal(el) {
            clearAllIntervals();
            var m=$('#retryModal'),b=$('#retryVerifyBtn'),c=10,t=10;
            m.css('display','flex'); b.prop('disabled',true).html('⏳ অপেক্ষা করুন...');
            $('#retrySeconds').text(c); $('#retryProgress').css('width','100%');
            ...
            retryInterval = setInterval(function(){ 
                c--; 
                $('#retrySeconds').text(c); 
                $('#retryProgress').css('width',((c/t)*100)+'%');
                if(c<=0){ 
                    clearAllIntervals();
                    b.prop('disabled',false).html('🔄 আবার ভেরিফাই করুন'); 
        ...
        $(document).on("click", ".copy", function() {
            let vl = $(this).attr('data-clipboard-text'), params = { 'type': 'text', 'value': vl };
