...
class Model extends BaseModel
{
    ...
    public function countResults($column, $table, $conditions)
    {
    ...
    public function getCountByType($table, $group, $conditions)
    {
        $query = $this->db->table($table)
            ->select("$group, COUNT(*) as total_count")
            ->groupBy($group)
            ->where($conditions)
            ->get();
    ...
    public function sumColumn($column, $table, $conditions)
    {
        return $this->db->table($table)
            ->selectSum($column)
            ->where($conditions)
    ...
    public function getSumByType($table, $group, $sum, $conditions)
    {
        $query = $this->db->table($table)
            ->select("$group, SUM($sum) as total_amount")
            ->groupBy($group)
            ->where($conditions)
            ->get();
    ...
    public function fetch($select = '*', $table = '', $where = '', $order = '', $by = 'DESC', $start = -1, $limit = 0, $return_array = false)
    {
        ...
        if (!empty($order) && (strtolower($by) === 'desc' || strtolower($by) === 'asc')) {
            if ($order === 'rand') {
    ...
    public function fetchByDate($select = '*', $table = '', $where = '', $date_column = 'created_at', $start_date = '', $end_date = '', $order = '', $by = 'DESC', $start = -1, $limit = 0, $return_array = false)
    {
        if (empty($start_date)) {
        ...
        $builder->where($date_column . ' >=', $start_date);

        if (!empty($end_date)) {
            $end_date = date('Y-m-d', strtotime($end_date));

            $builder->where($date_column . ' <=', $end_date);
        }
    ...
    public function sendMail($subject, $email_content, $user_id_or_email, $user_type = 'user', $check_replace = true)
    {
        $email_from = get_option('email_from', '') ?: "support@ekhonidigital.com";
        $email_name = get_option('email_name', '') ?: get_option('site_title', 'Ekhoni Digital');

        $website_link = base_url();
        $website_logo = base_url(get_option('site_logo', "assets/images/logo.png"));
        $website_name = get_option("site_name", "Ekhoni Digital");
        $copyright = get_option('copy_right_content', "Copyright &copy; 2023");

        // Get recipient email and name based on user type
        if ($user_type == 'general') {
            $recipient_email_address = $user_id_or_email;
            $recipient_name = $user_id_or_email;
        } elseif ($user_type == 'user') {
            $user_info = $this->db->table('users')
                ->where('id', $user_id_or_email)
                ->orWhere('email', $user_id_or_email)
                ->get()->getRow();
            if (empty($user_info)) {
                return "Failed to send email template! User Account does not exist!";
            }
            $recipient_email_address = $user_info->email;
            $recipient_name = $user_info->first_name;
        } else {
            $user_info = $this->db->table('staffs')
                ->where('id', $user_id_or_email)
                ->orWhere('email', $user_id_or_email)
                ->get()->getRow();
            if (empty($user_info)) {
                return "Failed to send email template! Staff Account does not exist!";
            }
            $recipient_email_address = $user_info->email;
            $recipient_name = $user_info->first_name;
        }

        // Replace placeholders in subject and email content if needed
        if ($check_replace && isset($user_info)) {
            $placeholders = [
                '{{first_name}}' => $user_info->first_name,
                '{{last_name}}' => $user_info->last_name,
                '{{email}}' => $user_info->email,
                '{{balance}}' => $user_info->balance,
                '{{activation_link}}' => base_url("activation/" . $user_info->activation_key),
                '{{website_name}}' => $website_name,
                '{{recovery_password_link}}' => base_url("password-reset/" . $user_info->reset_key),
                '{{admin_recovery_password_link}}' => base_url("admin/password-reset/" . $user_info->reset_key),
        ...
        $html = view('layouts/mail', $data);

        $email = service('email');

        $email->setFrom($email_from, $email_name);
        $email->setReplyTo($email_from, $email_name);
        $email->setTo($recipient_email_address);
        $email->setSubject($subject);
        $email->setMessage($html);
        $email->setMailType('html');

        try {
            $email_protocol_type = get_option("email_protocol_type", "mail");
            if ($email_protocol_type == "smtp") {
                ...
                if ($smtp_server && $smtp_port && $smtp_username && $smtp_password) {
                    ...
                    $email->setSMTPHost($smtp_server);
                    $email->setSMTPPort($smtp_port);
                    $email->setSMTPUser($smtp_username);
                    $email->setSMTPPass($smtp_password);
                    $email->setSMTPCrypto($smtp_encryption);
                }
            }
    ...
    function setLogs($task, $user = "admin"): void
    {
        $insData = [
            'ip'         => ip(),
            'activity'   => $task ?? 'Failed to Detect',
            'created_at' => Time::now()
        ];
        if ($user == "admin") {
            $insData["uid"] = session("sid");
            $this->db->table('admin_activity_logs')->insert($insData);
        } else {
            $insData["uid"] = session("uid");
            $this->db->table('activity_logs')->insert($insData);
        }
    }
}
